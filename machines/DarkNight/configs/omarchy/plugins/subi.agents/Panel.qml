...
Panel {
  id: root
  moduleName: "subi.agents"
  ipcTarget: "subi.agents"
  manageIpc: false
  ...
  readonly property bool alarming: (!!headline && (headline.isRemaining ? headline.percent <= 0.1 : headline.percent >= 0.9)) || balanceAlarming
  ...
  function limitWindow(label, percent, resetAt, title, remaining) {
    return {
      title: String(title || "") !== "" ? String(title) : windowTitle(label),
      percent: Number(percent),
      resetAt: String(resetAt || ""),
      isRemaining: remaining === true || /remaining/i.test(label) || /remaining/i.test(title)
    }
  }
  ...
  function limitWindows(p) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (percent >= 0) out.push(limitWindow(entry.label, percent, entry.resetsAt, entry.title, entry.isRemaining))
    }
    return out
  }

  // The window that decides how much room is left — the fullest/most depleted one
  function bindingWindow(p) {
    var windows = limitWindows(p)
    var best = null
    for (var i = 0; i < windows.length; i++) {
      var effectiveUsed = windows[i].isRemaining ? (1.0 - windows[i].percent) : windows[i].percent
      var bestUsed = best ? (best.isRemaining ? (1.0 - best.percent) : best.percent) : -1
      if (!best || effectiveUsed > bestUsed) best = windows[i]
    }
    return best
  }
  ...
  component LimitRow: Column {
    ...
    readonly property bool alarming: window && (window.isRemaining ? window.percent <= 0.1 : window.percent >= 0.9)
