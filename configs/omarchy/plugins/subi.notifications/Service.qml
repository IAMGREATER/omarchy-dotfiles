...
Item {
  ...
  // Toasts are fixed to the bottom-right corner. They clear the omarchy bar
  // when the bar occupies the bottom or right edge.
  ...
  function invokePopupDefault(index) {
    ...
    try {
      ...
      // Notification already torn down by the server — fall through to smart action.
      console.warn("invoke default failed:", e)
    }
    if (!invoked) performSmartAction(entry)
    dismissPopup(index)
  }

  function performSmartAction(entry) {
    if (!entry) return

    // 1. File path in entry.image if pointing to local file (e.g. screenshot)
    if (entry.image && String(entry.image).indexOf("file://") === 0) {
      var imgPath = String(entry.image).replace(/^file:\/\//, "")
      if (imgPath && imgPath.charAt(0) === "/") {
        if (entry.summary && /screenshot/i.test(entry.summary)) {
          Util.execArgv(["xdg-open", imgPath])
          return
        }
      }
    }

    var fullText = (String(entry.summary || "") + " " + String(entry.body || "")).trim()

    // 2. Crash notification -> AI Diagnosis
    if (/process crashed/i.test(entry.summary) || /diagnose with ai/i.test(fullText)) {
      Util.execArgv(["bash", "-c",
        "pid=$(coredumpctl list --no-legend 2>/dev/null | tail -1 | awk '{print $5}'); " +
        "if [[ -n $pid && $pid =~ ^[0-9]+$ ]]; then omarchy-agent-crash \"$pid\"; " +
        "else omarchy-agent --prompt 'Diagnose the most recent crash on this machine'; fi"])
      return
    }

    // 3. Local file path in summary or body
    var filePathMatch = fullText.match(/(?:file:\/\/)?(\/(?:home|tmp|usr|etc|var)[^\s"'<>:\0\n\r]+\.[a-zA-Z0-9_-]+)/)
    if (filePathMatch && filePathMatch[1]) {
      Util.execArgv(["xdg-open", filePathMatch[1]])
      return
    }

    // 4. Web URL in body or summary
    var urlMatch = String(entry.body || "").match(/https?:\/\/[^\s"'<>]+/i) || String(entry.summary || "").match(/https?:\/\/[^\s"'<>]+/i)
    var url = urlMatch ? urlMatch[0] : ""
    var appName = String(entry.app || "").trim()

    // 5. WhatsApp specific check: focus or launch WhatsApp web app installed on Omarchy
    if (/web\.whatsapp\.com|whatsapp/i.test(fullText) || /web\.whatsapp\.com|whatsapp/i.test(url) || /whatsapp/i.test(appName)) {
      Util.execArgv(["omarchy-launch-or-focus-webapp", "WhatsApp", "https://web.whatsapp.com/"])
      return
    }

    // 6. General web apps / installed desktop webapps (e.g. Google Messages, Discord, Teams, Basecamp, etc.)
    if (url) {
      var domainMatch = url.match(/https?:\/\/([^/\s]+)/i)
      var domain = domainMatch ? domainMatch[1] : ""
      if (domain) {
        Util.execArgv([
          "bash", "-c",
          "domain=\"$1\"; url=\"$2\"; app=\"$3\"; " +
          "match=$(grep -l -E \"^Exec=.*(omarchy-launch-webapp|omarchy-webapp-handler|--app=).*$domain\" ~/.local/share/applications/*.desktop /usr/share/omarchy/applications/*.desktop /usr/share/applications/*.desktop 2>/dev/null | head -1); " +
          "if [[ -n $match ]]; then " +
          "  desktop=$(basename \"$match\" .desktop); " +
          "  omarchy-launch-or-focus-webapp \"$desktop\" \"$url\"; " +
          "elif [[ -n $app && ! $app =~ ^(Chromium|Google-chrome|google-chrome|Brave-browser|brave|opera|vivaldi|firefox|omarchy-action|notify-send|system)$ ]]; then " +
          "  omarchy-hyprland-focus-app \"$app\" 2>/dev/null || " +
    ...
    // 7. Desktop app focus (e.g. Slack, Spotify, Ghostty, Nautilus)
    var isGenericOrBrowser = (!appName || /^(omarchy-action|notify-send|system|Chromium|Google-chrome|google-chrome|Brave-browser|brave|opera|vivaldi|firefox)$/i.test(appName))
    if (!isGenericOrBrowser && /^[A-Za-z0-9][A-Za-z0-9 ._-]{0,63}$/.test(appName)) {
      Util.execArgv([
        "bash", "-c",
        "omarchy-hyprland-focus-app \"$1\" 2>/dev/null || " +
        "gtk-launch \"$1\" 2>/dev/null || " +
    ...
    // 7. Omarchy system widgets / topics
    if (/weather/i.test(appName) || /weather|forecast|rain|cloud|temp/i.test(fullText)) {
      Util.execArgv(["omarchy-shell", "shell", "togglePanel", "omarchy.weather"])
      return
    }
    if (/proton vpn|vpn/i.test(appName) || /vpn|wireguard/i.test(fullText)) {
      Util.execArgv(["omarchy-shell", "shell", "togglePanel", "io.github.grichard99.omaproton-vpn"])
      return
    }
    if (/reminder/i.test(fullText)) {
      Util.execArgv(["omarchy-shell", "shell", "summon", "omarchy.reminders", "{}"])
      return
    }
    if (/update|package/i.test(fullText)) {
      Util.execArgv(["omarchy-shell", "shell", "togglePanel", "dizziee.system-updates"])
      return
    }
    if (/battery|power/i.test(fullText)) {
      Util.execArgv(["omarchy-shell", "shell", "togglePanel", "omarchy.power"])
      return
    }
    if (/time|calendar|clock/i.test(fullText)) {
      Util.execArgv(["omarchy-shell", "shell", "togglePanel", "omarchy.clock"])
      return
    }
    if (/bluetooth/i.test(fullText)) {
      Util.execArgv(["omarchy-shell", "shell", "togglePanel", "omarchy.bluetooth"])
      return
    }
    if (/wi-fi|wifi|network/i.test(fullText)) {
      Util.execArgv(["omarchy-shell", "shell", "togglePanel", "omarchy.network"])
      return
    }
    if (/clipboard|copied/i.test(fullText)) {
      Util.execArgv(["omarchy-clipboard-open"])
      return
    }
    if (/theme|font/i.test(fullText)) {
      Util.execArgv(["omarchy-menu-themes"])
      return
    }
    if (/audio|volume|sound/i.test(fullText)) {
      Util.execArgv(["omarchy-shell", "shell", "togglePanel", "ssupt.audio-control"])
      return
    }
  }

  function focusApp(entry) {
    performSmartAction(entry)
  }
