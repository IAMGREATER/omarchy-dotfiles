...
Panel {
  ...
  function toggleDisplay(name, enabled) {
    ...
    var escapedName = String(name).replace(/\\/g, "\\\\").replace(/"/g, "\\\"")
    var powered = root.powerStates[name] !== false
    var nextStates = {}
    for (var monitorName in root.powerStates) nextStates[monitorName] = root.powerStates[monitorName]
    nextStates[name] = !powered
    root.powerStates = nextStates
    actionProc.command = [
      "hyprctl",
      "eval",
      "return hl.dispatch(hl.dsp.dpms({ state = \"" + (powered ? "off" : "on")
        + "\", monitor = \"" + escapedName + "\" }))"
    ]
  ...
  component MonitorRow: CursorSurface {
    ...
    Row {
      ...
      Text {
        ...
        elide: Text.ElideRight
        width: parent.width - Style.space(22) - Style.space(14)
          - monitorPowerSwitch.implicitWidth - Style.space(24)
        anchors.verticalCenter: parent.verticalCenter
      }
      ...
      ToggleSwitch {
        id: monitorPowerSwitch
        z: 1
        checked: root.powerStates[monitorRow.display.name] !== false
    ...
    MouseArea {
      ...
      onClicked: if (monitorRow.canToggle && !monitorPowerSwitch.containsMouse)
