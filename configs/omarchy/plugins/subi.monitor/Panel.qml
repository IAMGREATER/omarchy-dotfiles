...
Panel {
  ...
  function toggleDisplay(name, enabled) {
    ...
    actionProc.command = ["omarchy-display-toggle", name, enabled ? "off" : "on"]
  ...
  component MonitorRow: CursorSurface {
    ...
    Row {
      ...
      Text {
        ...
        elide: Text.ElideRight
        width: parent.width - Style.space(22) - powerSwitch.implicitWidth - Style.space(16)
        anchors.verticalCenter: parent.verticalCenter
      }

      ToggleSwitch {
        id: powerSwitch
        checked: !!(monitorRow.display && monitorRow.display.enabled)
        enabled: monitorRow.canToggle
        foreground: root.bar.foreground
        anchors.verticalCenter: parent.verticalCenter
        onToggled: if (monitorRow.canToggle) root.toggleDisplay(monitorRow.display.name, monitorRow.display.enabled)
      }
    }
